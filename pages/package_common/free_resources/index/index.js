...
Page({
  ...
  onLoad: function() {
    ...
    setTimeout(() => {
      ...
      console.log(this.eachRightItemToTop)
    }, 500)
  },
  ...
  getEachRightItemToTop: function() {
    try {
      ...
      for (let i = 0; i < constants.length; i++) {
        let rowCount = Math.ceil(constants[i].category.length / 3)
        ...
        obj[constants[i + 1] ? constants[i + 1].id : 'last'] = totop
      }
      return obj
    } catch (error) {
      console.log(error)
  ...
  rightScroll: function(e) {
    for (let i = 0; i < this.data.constants.length; i++) {
      let tempId = this.data.constants[i].id,
        tempNextId = this.data.constants[i + 1],
